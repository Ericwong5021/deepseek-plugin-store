...
- [Nagi-ovo/dsh-ads](https://github.com/Nagi-ovo/dsh-ads) ★166 · `@dsh-external/dsh-ads` — 是兄弟就来蹬我！DSH Web UI 广告：2005 年中文站点风格的侧栏广告 / 对话内信息流 / 角落弹窗 + 一个真实热区比视觉小得多的关闭叉。素材全虚构，域名打码。
- [huiliyi37/dsh-tianshu-tui](https://github.com/huiliyi37/dsh-tianshu-tui) ★85 · `@huiliyi37/dsh-tianshu-tui` — dsh-tianshu-tui — DeepSeek Harness terminal UI +harness workflow。是官方 DeepSeek Harness 上的交互式终端 UI 插件。渲染核心从本仓库自研的harness agent  Tianshu-Tui 演进而来，在官方的基础上增加了TDD与证据门等工作流。
- [hust-open-atom-club/oh-dsh](https://github.com/hust-open-atom-club/oh-dsh) ★83 · `@oh-dsh/desktop` — 一站式 DeepSeek Harness 社区发行版：TUI、桌面端与 Web UI 三种形态统一体验，支持分层安装、一步到位，免去手工整合打包。
- [Nagi-ovo/dsh-visualize](https://github.com/Nagi-ovo/dsh-visualize) ★44 · `@dsh-external/dsh-visualize` — DSH 对话内生成式 UI 插件：模型把交互式 HTML 卡片直接画进会话流——visualize 工具 + 配套 skill + 沙箱渲染卡，带流式预览、组件浮入动画与鲸鱼蓝主题跟随
- [ZSeven-W/dsh-openpencil](https://github.com/ZSeven-W/dsh-openpencil) ★41 · `@zseven-w/dsh-openpencil` — OpenPencil design preview and editing plugin for DSH
- [omdsh-dev/dsh-genui](https://github.com/omdsh-dev/dsh-genui) ★28 · `@omdsh-dev/dsh-genui` — GenUI for DeepSeek Harness: interactive UI components rendered inline in assistant replies via the dsh-ui fence — layout, charts, plots, forms, quizzes, mermaid, 3D scenes, and an action event loop back to the model. Ships the fence-teaching host plugin, the browser renderer (client half), and the genui skill.
- [alingalingling/ui-status-label](https://github.com/alingalingling/ui-status-label) ★23 · `dsh-ui-status-label` — 把你鲸鱼娘思考时的 deep diving 自定义成任意你想要的样子
...
- [ccq1/dsh-side-panel](https://github.com/ccq1/dsh-side-panel) ★9 · `@dsh-external/dsh-side-panel` — DSH 侧边栏，集成文件浏览器、终端和 Git 审查，方便预览文件。
- [lhh010/dsh-minigames](https://github.com/lhh010/dsh-minigames) ★9 · `@dsh-external/dsh-minigames` — DSH Web UI 右侧小游戏面板：18 款离线小游戏（恐龙跳一跳 / 俄罗斯方块 / 坦克大战 / 扫雷 / 2048 / 数独 / 吃豆人 / 跟枪练习等），可扩展游戏注册表，等待模型回复或修 bug 时的摸鱼神器
- [Nwflower/dsh-chat-import](https://github.com/Nwflower/dsh-chat-import) ★9 · `dsh-chat-import` — 从Claude Code、Codex、Reasonix等Agent工具导入历史消息，并在DSH中继续对话
...
- [omdsh-dev/dsh-gomoku](https://github.com/omdsh-dev/dsh-gomoku) ★7 · `@deepseek-ai/dsh-gomoku` — 在DSH中与AI下五子棋，也可以让AI对局，看哪个AI棋力更强
- [dsh-tui/dsh-tui](https://github.com/dsh-tui/dsh-tui) ★7 · `@dsh-tui/dsh-tui` — Claude Code-style terminal UI for DeepSeek Harness agents, as an out-of-tree dsh plugin bundle
...
- [HuanLinOTO/dsh-plugin-mineru](https://github.com/HuanLinOTO/dsh-plugin-mineru) ★5 · `@huanlin/dsh-plugin-mineru` — 向模型暴露 MinerU 文档解析工具，将 PDF/图片/DOCX/PPTX/XLSX 转为结构化 Markdown/JSON | Exposes MinerU document-parsing tools to the model, converting PDF/images/DOCX/PPTX/XLSX into structured Markdown/JSON
- [omdsh-dev/dsh-data-agent](https://github.com/omdsh-dev/dsh-data-agent) ★5 · `@deepseek-ai/dsh-data-agent` — 让AI帮你连数据库、写SQL的DSH插件
...
- [vlln/dsh-task-status](https://github.com/vlln/dsh-task-status) ★4 · `@dsh-external/dsh-task-status` — DSH 插件：后台任务状态条（对话页任务进度 + 实时输出 tail）。官方 bundle 插件，dsh plugin --profile web add 安装
...
- [THU-MAIC/dsh-openmaic](https://github.com/THU-MAIC/dsh-openmaic) ★4 · `@openmaic/dsh-openmaic` — OpenMAIC for DeepSeek Harness: classrooms, slides, interactive widgets, and Socratic teaching
...
- [xingyingyuzhui/dsh-updater-ui](https://github.com/xingyingyuzhui/dsh-updater-ui) ★3 · `dsh-updater-ui` — DeepSeek Harness 更新检查器：设置页一键检查/拉取 DSH 更新（git pull --ff-only），自动后台检查 + 红点提醒 + 版本对比 + 更新说明。DSH updater with one-click pull, auto-check, version diff and changelog preview.
...
- [turtle1999/turtle-ui](https://github.com/turtle1999/turtle-ui) ★3 · `@deepseek-ai/dsh-tui` — as is, no warranty
- [renat3u/dsh-web-archive](https://github.com/renat3u/dsh-web-archive) ★3 · `dsh-web-archive` — 折叠对话当中众多的“无用消息”，例如Think、Bash等
- [btspoony/dsh-advisor](https://github.com/btspoony/dsh-advisor) ★3 · `dsh-advisor` — Advisor - Pair a second model that passively reviews each turn and injects notes.  搭配一个会在每轮对话被动注入见解和审查的副模型。
...
- [omdsh-dev/dsh-lark](https://github.com/omdsh-dev/dsh-lark) ★2 · `dsh-lark-channel` — Lark/Feishu IM bot channel for DeepSeek Harness: chats drive agents, replies and approvals return as messages and cards
- [lehhair/dsh-diff-viewer](https://github.com/lehhair/dsh-diff-viewer) ★2 · `@dsh-external/dsh-diff-viewer` — DSH Web GUI PiUI-style diff viewer plugin: replaces the stock DiffBlock for write/edit tool calls via ui-tool diff-card chain slots (host patch included). Private.
...
- [anweat/dsh-web-search-pro](https://github.com/anweat/dsh-web-search-pro) ★2 · `dsh-web-search-pro` — Enhanced, persistent web search plugin for DeepSeek Harness (multi-engine search, SQLite+LRU cache, platform backends, Playwright rendering)
...
- [zjl88858/dsh-huadongbianzuqi](https://github.com/zjl88858/dsh-huadongbianzuqi) ★2 · `dsh-huadong-bianzuqi` — DeepSeek Harness的滑动变祖器插件
...
- [BrambleXu/dsh-annotate](https://github.com/BrambleXu/dsh-annotate) ★0 · `dsh-annotate` — Visual browser element annotation for DeepSeek Harness, capturing DOM, styles, accessibility data, comments, and viewport screenshots. DeepSeek Harness 浏览器元素标注插件，捕获 DOM、样式、可访问性数据、评论和视口截图。
- [lordship12138-crypto/dsh-plugin-dedup](https://github.com/lordship12138-crypto/dsh-plugin-dedup) ★0 · `dsh-plugin-dedup` — Review duplicate dynamic Cordis plugins and produce merge plans for DeepSeek Harness (DSH). Detects content-similar packages (token Jaccard/containment), clusters duplicates, and yields safe merge plans or superset final sources. Entirely AI-generated.
- [franksong2702/dsh-codex-connect](https://github.com/franksong2702/dsh-codex-connect) ★0 · `dsh-codex-connect` — ChatGPT OAuth and Codex models for DeepSeek Harness.
...
- [drfccv/dsh-theme-neko](https://github.com/drfccv/dsh-theme-neko) ★0 · `dsh-theme-neko` — A Nachoneko (甘城猫猫) themed skin for the DeepSeek Harness web GUI.
...
- [yequ172672/dsh-codex-subscription](https://github.com/yequ172672/dsh-codex-subscription) ★0 · `dsh-llm-codex` — DSH 插件:直接复用 Codex CLI 本地登录订阅凭证,在 DeepSeek Harness 中使用 ChatGPT 订阅模型,无需 API Key | DSH plugin: reuse your Codex CLI local subscription login to use ChatGPT subscription models in DeepSeek Harness, no API key required
...
- [zimzaza4/dsh-bash-win](https://github.com/zimzaza4/dsh-bash-win) ★0 · `@zimzaza4/dsh-bash-win` — 在 Windows 环境中为 DeepSeek Harness 提供 Git Bash 与 WSL2 bash 工具,含 bwrap 沙箱、审批模式、后台任务
...
- [titanwings/dsh-automation](https://github.com/titanwings/dsh-automation) ★14 · `@dsh-external/dsh-automation` — DSH 自动化插件：让 Coding 任务按计划在全新 Agent Session 中运行，并由用户或 Agent 创建和管理定时任务。 / Run coding tasks in fresh Agent sessions and manage schedules from DSH Web or an Agent.
...
- [omdsh-dev/dsh-deep-research](https://github.com/omdsh-dev/dsh-deep-research) ★5 · `@dsh-external/dsh-deep-research` — Adaptive deep-research orchestrator plugin for DeepSeek Harness (official workflow engine, cybernetics/information-theory design)
...
- [Areium/dsh-fail-logger](https://github.com/Areium/dsh-fail-logger) ★1 · `dsh-fail-logger` — DeepSeek Harness（DSH）插件：自动记录所有执行模式（原生工具 / PTC run_code / 代码内嵌工具调用）的工具失败错因，去重、计数、确定性排序后沉淀进 skill 的机器维护实录区段——让 Agent 越用越少错。
...
- [levi-qiao/dsh-plugin-longgraph](https://github.com/levi-qiao/dsh-plugin-longgraph) ★0 · `dsh-plugin-longgraph` — DeepSeek Harness community plugin: longgraph / loop-graph / loop-converge authoring skills on ctx.skills
...
- [omdsh-dev/dsh-at-file](https://github.com/omdsh-dev/dsh-at-file) ★53 · `dsh-at-file` — Codex-style @file mentions for DeepSeek Harness: search workspace files in the composer and attach their contents to prompts.
...
- [vlln/dsh-navbar](https://github.com/vlln/dsh-navbar) ★6 · `@dsh-external/dsh-navbar` — DSH 插件：对话节点导航条（右缘节点串快速跳转 user 消息）。官方 bundle 插件，dsh plugin --profile web add 安装
- [lhmd/dsh-director-toolkit](https://github.com/lhmd/dsh-director-toolkit) ★4 · `@lhmd/dsh-director-toolkit` — DSH Director Toolkit is a DeepSeek Harness plugin for 3D artists, technical designers, and creative coders. Paste a half-formed idea, a reference note, or a portfolio caption and get a compact direction pack for Blender, Three.js, Houdini, or C4D.
...
- [lhmd/dsh-promotion-toolkit](https://github.com/lhmd/dsh-promotion-toolkit) ★3 · `@lhmd/dsh-promotion-toolkit` — 把你的任何想法，变成每个平台原生的宣发内容 | Turn any idea into platform-native publicity
...
- [awesome-dsh-plugin/dsh-find-plugin](https://github.com/awesome-dsh-plugin/dsh-find-plugin) ★2 · `dsh-find-plugin` — Find DSH plugins inside the agent — live GitHub dsh-plugin topic search, star-ranked / 会话内搜索发现 DSH 插件
...
- [gxpppp/dsh-search-mcp](https://github.com/gxpppp/dsh-search-mcp) ★2 · `dsh-search-mcp` — Replace dsh's built-in web search with search MCP servers (Tavily/Brave/Exa/Perplexity/DuckDuckGo/custom), configured from the web Settings page. Disables the built-in DeepSeek search provider while enabled.
...
- [BrambleXu/dsh-prompt-profile](https://github.com/BrambleXu/dsh-prompt-profile) ★0 · `dsh-prompt-profile` — Reusable Markdown prompt profiles for DeepSeek Harness with per-turn model selection, argument substitution, and state restoration. DeepSeek Harness 可复用 Markdown Prompt Profile，支持单轮模型选择、参数替换和状态恢复。
...
- [Jelee0145/dsh-mem](https://github.com/Jelee0145/dsh-mem) ★0 · `dsh-mem` — Cross-session long-term memory for DeepSeek Harness (dsh): durable JSON-file memory store with memory_save / memory_recall / memory_forget / memory_list model tools, installable as a dsh bundle.
...
- [yuxino/dsh-blue-whale-maid](https://github.com/yuxino/dsh-blue-whale-maid) ★2 · `dsh-blue-whale-maid` — 蓝鲸女仆 ——（dsh web）的桌面宠物插件，本质是一个任务完成提醒器。
...
- [jelly-000/dsh-balance-monitor](https://github.com/jelly-000/dsh-balance-monitor) ★1 · `dsh-balance-monitor` — DeepSeek 账户余额、剩余比例条与今日花费，显示在 dsh 侧边栏底部 · DeepSeek balance, remaining-ratio bar and today's spend in the dsh sidebar footer.
- [STARDUSTLC666/dsh-slack](https://github.com/STARDUSTLC666/dsh-slack) ★0 · `dsh-slack` — DSH 社区插件：Slack 通知/桥接（v0.2 支持 Socket Mode 双向：收件箱 + 线程回复）
...
- [Anionex/dsh-turn-rewind](https://github.com/Anionex/dsh-turn-rewind) ★25 · `@dsh-external/turn-rewind` — deepseek harness对话和代码状态回退插件 | DSH — rewind conversation and workspace state, powered by a persistent Change Ledger
- [forrestchang/dsh-multica-runtime](https://github.com/forrestchang/dsh-multica-runtime) ★20 · `@multica-ai/dsh-runtime` — Support dsh runtime on Multica.
- [Chinesezjc/dsh-interconnect](https://github.com/Chinesezjc/dsh-interconnect) ★17 · `dsh-interconnect` — Cross-instance message/event handoff plugins for DSH (interconnect service + tools)
- [Moeblack/dsh-message-edit](https://github.com/Moeblack/dsh-message-edit) ★13 · `dsh-message-edit` — DSH plugin: branch-based message editing, reroll, retry, version timeline
- [Anionex/dsh-computer-use](https://github.com/Anionex/dsh-computer-use) ★13 · `@dsh-external/dsh-computer-use` — 为 DeepSeek Harness 提供电脑控制插件：新鲜 Accessibility 观测、过期状态拒绝、作用域权限与安全输入（目前支持macos）｜Accessibility-first macOS Computer Use bundle for DSH with fresh observations, stale-state rejection, scoped permissions, and safe input.
...
- [william-jin-cmu/dsh-stickers](https://github.com/william-jin-cmu/dsh-stickers) ★7 · `@dsh-external/dsh-stickers` — DSH WebUI sticker plugin for bidirectional user and agent reactions
- [AnacondaKC/dsh-stock-market](https://github.com/AnacondaKC/dsh-stock-market) ★6 · `dsh-stock-market` — 有效解决了写代码的时候账户不能同时亏钱的BUG
...
- [springbrand-lab/dsh-oauth-mcp-client](https://github.com/springbrand-lab/dsh-oauth-mcp-client) ★4 · `@dsh-external/dsh-oauth-mcp-client` — OAuth 2.1 Streamable HTTP MCP client plugin for DeepSeek Harness.
...
- [hyqhyq3/dsh-mcp-manager](https://github.com/hyqhyq3/dsh-mcp-manager) ★4 · `dsh-mcp-manager` — MCP server manager plugin for DeepSeek Harness: Settings → MCP page, OAuth (PKCE + dynamic client registration) or static-token auth, tools registered as mcp__<name>__*
...
- [Ghost011118/dsh-balance-meter](https://github.com/Ghost011118/dsh-balance-meter) ★4 · `dsh-balance-meter` — DeepSeek account balance and session cost readout for the DeepSeek Harness Web GUI
...
- [sliverp/DeepSeek-harness-qqbot](https://github.com/sliverp/DeepSeek-harness-qqbot) ★3 · `deepseek-harness-qqbot` — QQ Bot text and image channel plugin for DeepSeek Harness
...
- [keleus/deepseek-pet](https://github.com/keleus/deepseek-pet) ★3 · `deepseek-pet` — 在你的deepseek-harness上养一只吃白饭的大蓝鲸
...
- [HuanLinOTO/dsh-plugin-d399](https://github.com/HuanLinOTO/dsh-plugin-d399) ★2 · `@huanlin/dsh-plugin-d399` — 模型生成时右下角弹出小游戏菜单（Wordle/消消乐/192 款参数化小游戏，可拓展注册表） | Pops up a mini-game menu while the model generates (Wordle/Match-3/192 parametric mini-games, extensible registry)
...
- [MirDie/dsh-xai](https://github.com/MirDie/dsh-xai) ★1 · `dsh-xai` — xAI Grok SuperGrok / X Premium OAuth for DeepSeek Harness
...
- [lynkas/dsh-think-flow-flow](https://github.com/lynkas/dsh-think-flow-flow) ★0 · `dsh-think-flow-flow` — DeepSeek Harness client plugin: constant-rate typewriter reveal for assistant output and reasoning, with per-model gating.
...
- [dongsheng123132/dsh-lineage](https://github.com/dongsheng123132/dsh-lineage) ★0 · `dsh-lineage` — Content-addressed data and action lineage evidence for DeepSeek Harness
...
- [acosmi/dsh-session-supervisor](https://github.com/acosmi/dsh-session-supervisor) ★0 · `dsh-session-supervisor` — Durable, bounded lifecycle supervisor for live DeepSeek Harness sessions (community plugin)
...
- [MOLAaaaaaaa/dsh-seismicx](https://github.com/MOLAaaaaaaa/dsh-seismicx) ★0 · `dsh-seismicx` — DeepSeek Harness plugin for the SeismicX earthquake-catalog skill
...
- [aryswisnu/dsh-eval-regression](https://github.com/aryswisnu/dsh-eval-regression) ★0 · `dsh-eval-regression` — Deterministic, CI-safe golden-output evaluation for DeepSeek Harness
- [shyboy/dsh-k12-lesson-builder](https://github.com/shyboy/dsh-k12-lesson-builder) ★0 · `dsh-k12-lesson-builder` — DeepSeek Harness plugin for generating synchronized K12 English PPTX and DOCX lesson materials
...
- [dongsheng123132/dsh-recovery-proof](https://github.com/dongsheng123132/dsh-recovery-proof) ★0 · `dsh-recovery-proof` — Read-only recovery drill evidence for DeepSeek Harness
...
- [Devin-AXIS/iPolloWork](https://github.com/Devin-AXIS/iPolloWork) ★3688 — A next-generation, source-available AI workspace with a self-evolving agent runtime for editable code, design, presentations, websites, and video—a Codex alternative that integrates DeepSeek Harness for subagent delegation, combining iPolloWork’s complete AI workbench with DSH’s specialized agents and both plugin ecosystems in one workflow.
- [whiteguo233/OpenBiliClaw](https://github.com/whiteguo233/OpenBiliClaw) ★2059 — 本地私有、开源的跨平台 AI 内容发现 Agent：先理解你，再主动从 B站、小红书、抖音、YouTube、X、知乎、Reddit、微博等平台与开放 Web 寻找内容。（支持 deepseek harness 插件） | Local-first open-source cross-platform AI content discovery agent: understands you, then proactively finds content across Bilibili, Xiaohongshu, Douyin, YouTube, X, Zhihu, Reddit, Weibo and the open web.（support deepseek harness plugin）
- [paean-ai/deeptide](https://github.com/paean-ai/deeptide) ★1032 — Built by DeepSeek, for DeepSeek — a Swift-native macOS coding agent
- [nutshellai-tech/mobius](https://github.com/nutshellai-tech/mobius) ★929 — The first self-evolving open-source Agent OS, connecting your team, AI agents, devices, and compute
- [zhu1090093659/dsh-web-ui](https://github.com/zhu1090093659/dsh-web-ui) ★829 — Plugin and skin collection for DeepSeek Harness (DSH) Web UI - task board, git graph, right-side panel, remote mobile UI, pet, live token stats, and skin center.
- [hellowind777/helloagents](https://github.com/hellowind777/helloagents) ★662 — 一个自主的高级智能伙伴，不仅分析问题，更持续工作直到完成实现和验证。
- [Anionex/agent-vision-toolkit](https://github.com/Anionex/agent-vision-toolkit) ★631 — 为纯文本模型"看图“设计更好的视觉工具箱和技能，支持多图理解，图片问答，前端UI还原、GUI 自动化等，并可选无缝接入多个主流agent，直接识别粘贴图片｜ A vision toolkit and skill designed for text-only llms — image Q&A, long-screenshot OCR, frontend UI restoration, and GUI automation, with optional seamless integration for Codex, Claude Code, Pi, Oh My Pi, and OpenCode
- [sandbaseai/sandbase-harness](https://github.com/sandbaseai/sandbase-harness) ★564 — Open-source CMA-compatible agent runtime. Run multi-agent systems locally with any model (Ollama/vLLM/Claude/GPT), MCP tools, scenario templates, and a beautiful dashboard. One command start. Built for enterprise teams.
- [AdamPlatin123/awesome-dsh-plugins](https://github.com/AdamPlatin123/awesome-dsh-plugins) ★446 — 前部索引仓库（Radar）：自动扫描发现的所有 dsh 插件候选；经测试合格的将移入后序精选目录仓库
- [PM-Shawn/Abu-Cowork](https://github.com/PM-Shawn/Abu-Cowork) ★303 — Open-source alternative to Claude Cowork — a local-first AI agent desktop app · multi-model · self-evolving skills · privacy-first · multi-Harness roadmap · DeepSeek Harness integration in progress
- [linhay/harmony-next.skills](https://github.com/linhay/harmony-next.skills) ★301 — 🚀 Expert guidance for HarmonyOS NEXT (API 12+) development. Covers IDE operations, performance tuning, architecture (HAP/HAR/HSP), and automation testing.
- [alaliqing/claude-paper](https://github.com/alaliqing/claude-paper) ★283 — 📚 Claude Code plugin that automates research papers study with automatic material generation, code demonstrations, and interactive web viewer.
- [cofy-x/axern](https://github.com/cofy-x/axern) ★253 — Open-source sandboxes for AI agents, untrusted code execution, and durable services.
- [morluto/rea](https://github.com/morluto/rea) ★246 — Reverse engineer anything with agents, from app behavior down to native binaries.
- [SepineTam/mcp-for-stata](https://github.com/SepineTam/mcp-for-stata) ★239 — A MCP server for Stata to integrate Stata into your agent.
- [openma-ai/open-managed-agents](https://github.com/openma-ai/open-managed-agents) ★232 — Open-source Claude Managed Agents API implementation and self-hosted Claude Tag-style agent runtime. Drop-in compatible; runs on Cloudflare Workers/Durable Objects or Node.js. Apache 2.0.
- [0xsline/awesome-deepseek-harness](https://github.com/0xsline/awesome-deepseek-harness) ★208 — DeepSeek Harness (DSH) ecosystem: curated plugins, tools, and infrastructure from dsh-external/hub and the public dsh-plugin topic.
- [awesome-dsh-plugin/awesome-dsh-plugin](https://github.com/awesome-dsh-plugin/awesome-dsh-plugin) ★207 — A curated list of plugins for DeepSeek Harness (dsh) · DeepSeek Harness 插件精选列表
- [Small-tailqwq/dsh-deep-whale](https://github.com/Small-tailqwq/dsh-deep-whale) ★196 — DSH Web 鲸鱼娘皮肤系列(深海女仆工坊 maid-atelier)——CC BY-NC-SA 4.0
- [humblebanana/open-record-replay](https://github.com/humblebanana/open-record-replay) ★134 — Open-source macOS record-and-replay workflow recorder for computer use agents. Captures mouse, keyboard, and UI events as structured traces so agents can learn, replay, and automate real desktop tasks.
- [zhaoolee/notes](https://github.com/zhaoolee/notes) ★133 — 开源版锤子便签，复刻锤科美学，一键Docker私有化部署，支持skill调用，支持多租户，一键生成公众号格式，支持导出便签为图片
- [pulseaiclub/phi](https://github.com/pulseaiclub/phi) ★72 — a coding Agent from pi. ∞ providers, sub-agents, hashline edits, and a permission gate, deepseek-tui
- [taxueseek/argo](https://github.com/taxueseek/argo) ★59 — 专门为 agent 打造的 agent 搜索工具，具备多语言搜索能力，覆盖中文/英文/学术/代码/购物/金融/新闻/百科。
- [Electricitysheep/dsh-handbook](https://github.com/Electricitysheep/dsh-handbook) ★56 — DeepSeek Harness (dsh) 从 0 到 1 深度手册：安装/插件开发/性能调优/实测案例/同模型多 Agent 实测对比（中文 + 英文 PDF）
...
- [Nagi-ovo/dsh-find-plugins](https://github.com/Nagi-ovo/dsh-find-plugins) ★42
...
- [Lum1104/dsh-browser](https://github.com/Lum1104/dsh-browser) ★39 — dsh plugin: Chrome sidebar extension that lets DSH operate your browser directly—no vision capabilities required.
...
- [Alex-Yanggg/awesome-DSH-plugin](https://github.com/Alex-Yanggg/awesome-DSH-plugin) ★36 — A meticulously curated list of useful plugins, extensions, tools and development resources built for DSH, covering productivity enhancement, functional expansion, debugging utilities and custom development modules.
- [Ruler4396/dsh-launcher](https://github.com/Ruler4396/dsh-launcher) ★35 — Lightweight Windows launcher for DeepSeek Harness: silent autostart at logon + a minimal WebView2 window instead of a full browser
- [bruc3van/awesome-dsh-plugin](https://github.com/bruc3van/awesome-dsh-plugin) ★33 — 用 30 秒找到适合你的 DeepSeek Harness 插件。 不只是仓库列表：这里告诉你插件解决什么问题、适合谁，以及从哪里开始。
...
- [LaplaceYoung/oh-my-dsh](https://github.com/LaplaceYoung/oh-my-dsh) ★28 — oh-my-dsh：面向 DSH (DeepSeek Harness) 的插件生态——700+ 插件，只通过扩展接缝注册，不修改 agent-loop 骨架
- [hanelalo/browser-bridge](https://github.com/hanelalo/browser-bridge) ★24 — 让你的agent真的像你一样操控你的浏览器窗口
...
- [zenx0x/allinluna](https://github.com/zenx0x/allinluna) ★23 — Resource-aware multi-agent orchestration for Codex and DeepSeek Harness (All in Flash DSH plugin)
- [csyangwen/dsh-memory-evolve](https://github.com/csyangwen/dsh-memory-evolve) ★22 — 为 DeepSeek Harness 带来「跨会话长期记忆 + 后台自我进化」能力的纯插件实现：五轨记忆 · git 分支感知 · 回合内自我审查 · 技能自我进化与技能管理器 · 四轨待办 · COI 调度 · 会话广播 · 会话搜索 · 提示词管理器 · 临时信息便签——零核心修改、零运行时依赖，随装随用、卸载即净。
- [ali-meoo/meoo-cli](https://github.com/ali-meoo/meoo-cli) ★20 — meoo cli 是秒悟（Meoo）官方推出的命令行工具，让 Claude Code、Codex、Cursor、Qoder等本地 agent 在帮你写完前端代码后，能直接接管「数据库、用户登录、文件存储、部署上线」的所有云端工作——你只需要在终端跑一条命令，剩下的交给 AI。
- [vlln/plugin-registry](https://github.com/vlln/plugin-registry) ★20 — DSH 插件生态基建：薄控制台（浏览器面板管理官方 repository 插件，0 patch）+ make-dsh-plugin skill 官方插件开发引导
- [Dominic789654/awesome-deepseek-harness](https://github.com/Dominic789654/awesome-deepseek-harness) ★18 — A curated list of plugins, skills, MCP servers, orchestrators & UIs for DeepSeek Harness (DSH). Visualization · PPT · Coding · Agents · Loops (auto-research) and more. #dsh
- [lhh010/dsh-ui-whale](https://github.com/lhh010/dsh-ui-whale) ★18 — 【求⭐】🐋DSH Web UI 全手绘像素鲸鱼伙伴插件：会话标题栏常驻，平时眨眼/偶尔摆尾/动胸鳍，思考运行时持续动起来，回合完成头顶喷水，点击还会冒爱心，不工作时还会偷懒睡觉，零核心改动。 【喜欢的话就点点star⭐吧~】
...
- [whiteguo233/dsh-openbiliclaw](https://github.com/whiteguo233/dsh-openbiliclaw) ★15 — OpenBiliClaw 是本地运行的跨平台个性化内容推荐 Agent，持续理解你的兴趣并主动找内容。本仓库是它的 DeepSeek Harness 插件：DSH 界面常驻第四栏（推荐/内容库/对话/画像/设置），注册 22 个 Agent Bridge 工具，让 Agent 也能读推荐、答探测、闭环学习。
...
- [wess09/DeepSeekHarnessDesktop](https://github.com/wess09/DeepSeekHarnessDesktop) ★11 — DeepSeekHarness桌面端打包
- [william-jin-cmu/dsh-vision](https://github.com/william-jin-cmu/dsh-vision) ★11 — dsh 插件：给纯文本 DeepSeek 加视觉——view_image 工具桥接任意 OpenAI 兼容 VLM（默认智谱免费档，实测 4 厂商 10 模型）
- [ChisaAlter/Deepseek-Harness-Desktop](https://github.com/ChisaAlter/Deepseek-Harness-Desktop) ★10 — Electron desktop shell for DeepSeek Harness web UI
...
- [bruc3van/dsh-desktop](https://github.com/bruc3van/dsh-desktop) ★8 — DeepSeek Harness Desktop 是一款社区维护的非官方第三方桌面客户端，通过直接加载官方 Web UI，为普通用户提供开箱即用的独立桌面体验：它可以自动复用本机已运行的官方实例，也可以使用安装包内置的 dsh 运行时启动服务，无需用户额外安装 Node.js 或 CLI，并提供智能连接、远程实例连接、托盘常驻、运行时监护和异常恢复等桌面增强。
- [pingfanfan/hello-dsh](https://github.com/pingfanfan/hello-dsh) ★8 — 从零开始，看懂 DeepSeek Harness 的「万物皆可插件」— 零基础插件开发教程（含 22 个中文技能实例）| Zero-to-plugin tutorial for DeepSeek Harness
...
- [ysr666/dsh-vision-router](https://github.com/ysr666/dsh-vision-router) ★8 — DeepSeek Harness plugin: turn-level vision routing with provider fallback chains, a cached vision_describe tool with JSON output and image downscaling, and optional per-host proxy support.
- [libukai/awesome-deepseek-harness](https://github.com/libukai/awesome-deepseek-harness) ★8 — DeepSeek Harness 终极指南：快速入门、资源推荐、精选插件与实用工具 ｜The Ultimate Guide to DeepSeek Harness: QuickStart, Resources, Plugins&Toolkit
...
- [ginuim/multi-screen-wireframe](https://github.com/ginuim/multi-screen-wireframe) ★5 — Generate offline multi-screen wireframes with canvas + demo navigation. Zero Node/npm.
...
- [Fisfzy/ego-browser](https://github.com/Fisfzy/ego-browser) ★5 — DSH（DeepSeek Harness）插件：把 ego-lite 浏览器（给 AI Agent 用的 Chromium）接入 HARNESS——13 个结构化 ego_* 工具（文本语义快照、语义定位点击、表单填充、截图、CDP 控制、任务空间隔离），内置 ego 运行时，Linux + Chrome 开箱即用，无需克隆官方仓库或手动构建。
...
- [bradeGithub/DSH-Plugins-Marketplace](https://github.com/bradeGithub/DSH-Plugins-Marketplace) ★5 — DSH插件市场 / DSH Plugin Marketplace: 在 DeepSeek Harness Web GUI 中一键浏览、安装与更新 GitHub topic:dsh-plugin 的全部插件 | browse, install & update all GitHub dsh-plugin plugins in the DSH Web GUI
...
- [like-study1/Oh-My-DSH](https://github.com/like-study1/Oh-My-DSH) ★4 — 🐳 DeepSeek Harness 插件聚合社区 — 自动同步 dsh-plugin 生态 · 精选目录 · 每 8 小时自动维护 | Oh-My-DSH: a community-maintained catalog of DeepSeek Harness plugins, auto-synced from the dsh-plugin topic
...
- [147228/dsh-xiaoyao-skins](https://github.com/147228/dsh-xiaoyao-skins) ★4 — 夕小瑶 × DeepSeek Harness Web 皮肤合集、安装器与社区创作工具链
...
- [DDDFXYqiming/Agent_Extensions](https://github.com/DDDFXYqiming/Agent_Extensions) ★4 — Agent Skills & DeepSeek Harness (DSH) 扩展库：通用智能体技能（General_skills）+ DSH 标准插件（dsh-plugin），开箱即用的 AI Agent 能力增强集合。
...
- [runzhliu/deepseek-harness-docker](https://github.com/runzhliu/deepseek-harness-docker) ★3 — Community Docker and Kubernetes packaging for DeepSeek Harness (@deepseek-ai/dsh), with a hardened image, Compose stack, Helm chart, Web UI, and headless CLI.
- [opensetk/dsh-xiaohei](https://github.com/opensetk/dsh-xiaohei) ★3 — dsh的罗小黑插件
- [FlashingChen/dsh-worktree](https://github.com/FlashingChen/dsh-worktree) ★3 — Codex-style permanent git worktrees for DeepSeek Harness: worktree_create/list/remove agent tools, a /worktree chat command, and durable per-repo manifests.
...
- [chyra-moon/deepseek-harness-desktop](https://github.com/chyra-moon/deepseek-harness-desktop) ★3 — DeepSeek Harness desktop shell: 1:1 replica of the official web UI as a Windows desktop app (community project)
...
- [Tyan66666/billion-context-dsh](https://github.com/Tyan66666/billion-context-dsh) ★3 — Model-driven context management (Active Context Pruning / ACP) for the DeepSeek Harness — the model decides when and what to compress. Ported from billion-context-pi (ranxianglei); acp-kernel reused verbatim. CompactionEngine backend with compress/decompress/search_context/acp_status tools.
...
- [TYEclipse/dsh-webfetch](https://github.com/TYEclipse/dsh-webfetch) ★1 — Web page reader for DeepSeek Harness (dsh): fetch any URL and extract clean Markdown / plain text plus a link inventory - zero runtime dependencies, read-only
- [omdsh-dev/dsh-github-integration](https://github.com/omdsh-dev/dsh-github-integration) ★1
- [Scorp1o117/dsh-tool-vision](https://github.com/Scorp1o117/dsh-tool-vision) ★1 — Vision model for DeepSeek Harness | DeepSeek Harness 外置视觉模型插件
...
- [hyls9527/dsh-plugins](https://github.com/hyls9527/dsh-plugins) ★1 — Ecosystem plugins for DeepSeek Harness: bounded cross-session memory and skill lifecycle curation, ported from hermes-agent. Tagged dsh-plugin.
...
- [modusensus/dsh-mneme](https://github.com/modusensus/dsh-mneme) ★1 — Mneme——把记忆主权还给人的记忆插件：SQLite + 可人工编辑的 Markdown 双写，autoDream 在梦境中巩固记忆，106 个测试护航。
...
- [coderPerseus/dsh-hub](https://github.com/coderPerseus/dsh-hub) ★1 — Cloudflare full-stack pnpm monorepo
...
- [NoWint/Oh-My-DSH](https://github.com/NoWint/Oh-My-DSH) ★1 — 🐋 DeepSeek Harness 插件精选集 · 300+ dsh-plugin 收录 · 22 大分类
- [Ychris12138/dsh-usage-stats](https://github.com/Ychris12138/dsh-usage-stats) ★1 — Token usage heatmap, per-model breakdowns, and DeepSeek account balance for the DeepSeek Harness Web GUI (dsh web).
- [JustGenius-s/DSH-Decktop](https://github.com/JustGenius-s/DSH-Decktop) ★1 — DSH-Decktop
...
- [JustGenius-s/DSH-Plugs](https://github.com/JustGenius-s/DSH-Plugs) ★1 — DSH Plugins Cellection
...
- [AlliotTech/deepseek-harness-docker](https://github.com/AlliotTech/deepseek-harness-docker) ★1 — deepseek-harness docker部署
...
- [NEXTINDIE/DeepSeek-Harness-for-VS-Code](https://github.com/NEXTINDIE/DeepSeek-Harness-for-VS-Code) ★1 — Use DeepSeek Harness in VS Code like ChatGPT/Copilot: @dsh in native chat, standalone views, cross-project sessions, shared via DSH API. Auto-starts server.
...
- [Beants/dsh-trellis](https://github.com/Beants/dsh-trellis) ★1 — Trellis workflow integration for DeepSeek Harness: Trellis skills, /trellis commands, and .trellis scaffolding
...
- [Scorp1o117/dsh-tdai-memory](https://github.com/Scorp1o117/dsh-tdai-memory) ★1 — Agent memory for DeepSeek Harness | DeepSeek Harness 记忆插件
...
- [Scorp1o117/dsh-soul-md](https://github.com/Scorp1o117/dsh-soul-md) ★1 — Soul.md persona for DeepSeek Harness | DeepSeek Harness 人设卡插件
...
- [weijiafu14/pi2dsh](https://github.com/weijiafu14/pi2dsh) ★0 — Bridge the Pi and DeepSeek Harness ecosystems: one Pi Host ABI runs unmodified Pi extensions as native DSH plugins. 打通 Pi 与 DSH 生态。
- [hrhgit/deepseek-harness-plugin-manager](https://github.com/hrhgit/deepseek-harness-plugin-manager) ★0 — Web plugin manager for DeepSeek Harness (DSH): inspect, search, group, enable, and disable Cordis plugins.
...
- [anweat/dsh-plugin-dev-guide](https://github.com/anweat/dsh-plugin-dev-guide) ★0 — DSH (DeepSeek Harness) plugin development & publishing guide: from first plugin to auto-publish to the community
...
- [hccccc01333/dsh-excel-vera-plugin](https://github.com/hccccc01333/dsh-excel-vera-plugin) ★0 — VERA — Verified Excel Reasoning Agent (dsh plugin): detects silent Excel formula errors, compiles semantic Formula IR, repairs deterministically or with LLM, scores against oracle workbooks, and validates charts.
...
- [PerryLink/dsh-background-agents](https://github.com/PerryLink/dsh-background-agents) ★0 — Interactive long-session background agents for DeepSeek Harness: start a durable continuable child agent, watch its progress in the Web UI sidebar, message it any time, and interrupt it - all through the official subagent seam.
...
- [Loong-wql/Build-HOS-mcp](https://github.com/Loong-wql/Build-HOS-mcp) ★0 — 适用于Codex的鸿蒙应用开发插件（HarmonyOS application development plugin ）
- [Deklan-Deng/Dcode](https://github.com/Deklan-Deng/Dcode) ★0 — Deepseek-harness 桌面端
...
- [t1yOS/t1y-skills](https://github.com/t1yOS/t1y-skills) ★0 — expert skill package for AI coding assistants working with the t1yOS Serverless platform.
...
- [Jinsong-Zhou/dsh-html-canvas](https://github.com/Jinsong-Zhou/dsh-html-canvas) ★0 — A DeepSeek Harness plugin that turns AI-generated HTML into a click-to-edit canvas beside the chat
...
- [kelai141/dsh-host-web-compat](https://github.com/kelai141/dsh-host-web-compat) ★0 — dsh 宿主插件——经 webserver 钩子向页面注入旧内核浏览器 polyfill。
...
- [chajiuqqq/dsh-claude-theme](https://github.com/chajiuqqq/dsh-claude-theme) ★0 — dsh的claude风格界面
- [Lixiaoyiao/deepseek-harness-action](https://github.com/Lixiaoyiao/deepseek-harness-action) ★0 — Community GitHub Action for DeepSeek Harness — AI Code Review · CI Diagnosis · Auto Fix · Issue → PR
...
- [kalynnka/vscode-deepseek-harness](https://github.com/kalynnka/vscode-deepseek-harness) ★0 — Unofficial: drive your own DeepSeek Harness (dsh) as a native VS Code chat agent, beside Claude Code and Codex.
...
- [sakurarain1213/deepseek-harness-lite](https://github.com/sakurarain1213/deepseek-harness-lite) ★0 — A lightweight, local-first distribution and verified plugin kit for DeepSeek Harness.
- [ZASENJC/dsh-plugins-store](https://github.com/ZASENJC/dsh-plugins-store) ★0 — 自动收录和分类 GitHub dsh-plugin Topic 项目的静态目录网站
...
- [Luaphes/dsh-plugins-market](https://github.com/Luaphes/dsh-plugins-market) ★0
...
- [sandbaseai/sandbase-skills](https://github.com/sandbaseai/sandbase-skills) ★0
...
- [junhongchashui/dsh-plugin-web-access](https://github.com/junhongchashui/dsh-plugin-web-access) ★0 — 纯本地的 DSH 按需网页访问插件：web_fetch 命令行抓取 + 无头浏览器（open/snapshot/eval/screenshot）双通道，免费、无需 API Key | Local-first web access plugin for DeepSeek Harness
...
- [kelai141/dsh-mobile-apk](https://github.com/kelai141/dsh-mobile-apk) ★0 — dsh 安卓壳 APK——WebView UI + 内嵌 Termux 运行时快照（解压即跑）、SAF 目录桥、保活服务、看门狗、运行时在线更新。
- [ycp424c/dsh-browser-bridge](https://github.com/ycp424c/dsh-browser-bridge) ★0 — Prompt-scoped bridge between DSH and explicitly attached Chrome tabs
