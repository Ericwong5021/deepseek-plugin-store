...
function decodeRequest(rawInput) {
    ...
    if (typeof payload !== 'object' || payload === null || Array.isArray(payload))
        throw new Error('安装请求内容无效');
    const record = payload;
    const profile = record.profile;
    const specs = record.specs;
    if (typeof profile !== 'string' || !profilePattern.test(profile))
...
async function installPlugins(ctx, rawInput) {
    ...
    if (entry === undefined || entry === '')
        throw new Error('无法定位当前 DSH 命令入口');
    ...
    if (outcome.exitCode !== 0)
        throw new Error(stderr || stdout || `dsh plugin 安装失败（退出码 ${String(outcome.exitCode)}）`);
