...
export function StorePage({ connected, installPlugins }) {
    ...
    useEffect(() => {
        ...
        if (frame === null)
            return;
        const sendReady = () => frame.contentWindow?.postMessage({ source: 'deepseek-dsh', type: 'dsh-store-capabilities', install: connected }, storeOrigin);
    ...
    return (_jsx("main", { className: css.page, "aria-label": "DeepSeek Plugin Store", children: _jsxs("div", { className: css.frameWrap, children: [_jsx("iframe", { ref: frameRef, className: css.frame, src: storeUrl, title: "DeepSeek Plugin Store", allow: "clipboard-write", sandbox: "allow-downloads allow-forms allow-popups allow-popups-to-escape-sandbox allow-same-origin allow-scripts", referrerPolicy: "strict-origin-when-cross-origin", onLoad: () => frameRef.current?.contentWindow?.postMessage({ source: 'deepseek-dsh', type: 'dsh-store-capabilities', install: connected }, storeOrigin) }), installing && _jsx("div", { className: css.installing, role: "status", children: "\u6B63\u5728\u5B89\u88C5\u63D2\u4EF6\u5230 DSH\u2026" })] }) }));
}
