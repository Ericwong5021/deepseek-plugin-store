...
export const validateRegistry = async () => {
  ...
  for (const [id, classification] of Object.entries(classifications.classifications)) {
    ...
    if (classification.summaries && (typeof classification.summaries.zh !== 'string' || !classification.summaries.zh.trim() || typeof classification.summaries.en !== 'string' || !classification.summaries.en.trim())) throw new Error(`invalid LLM plugin summaries: ${id}`)
    if (classification.cacheKey && (!/^[a-f0-9]{40}$/.test(classification.repositoryCommitSha || '') || !/^[a-f0-9]{64}$/.test(classification.cacheKey) || !/^[a-f0-9]{64}$/.test(classification.taxonomyHash || '') || !classification.policyVersion || !classification.risk || !classification.governance || typeof classification.shadowMode !== 'boolean')) throw new Error(`invalid governance classification metadata: ${id}`)
  }
